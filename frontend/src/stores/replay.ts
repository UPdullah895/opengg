...
import { ref, computed, shallowRef, triggerRef, markRaw, reactive } from 'vue'
...
function compareNewestFirst(a: Clip, b: Clip): number {
  return b.created.localeCompare(a.created)
    || (b.createdTs - a.createdTs)
    || b.filename.localeCompare(a.filename)
}

function compareOldestFirst(a: Clip, b: Clip): number {
...
export const useReplayStore = defineStore('replay', () => {
  ...
  const liveThumbs = reactive(new Map<string, string>())
  const liveMeta = reactive(new Map<string, { duration: number; width: number; height: number }>())
  const loading = ref(false)
  const loaded = ref(false)
  const clipsProbed = ref(false)
  ...
  const filteredRealClips = computed(() => {
    const real = clips.value.filter(c => !c.isSkeleton)
  ...
  const filteredClips = computed(() => {
    ...
    const result = [...skeletons, ...filteredRealClips.value]
    if (import.meta.env.DEV) {
  ...
  function applyProbeAndThumb(fp: string, dur: number, w: number, h: number, thumbId: string, thumbPath: string, opts?: { triggerClips?: boolean; deferFlush?: boolean }) {
    if (dur > 0) liveMeta.set(thumbId, { duration: dur, width: w, height: h })
    liveThumbs.set(thumbId, thumbPath)

    const i = clips.value.findIndex(c => c.filepath === fp)
    if (i >= 0) {
      const prev = clips.value[i]
      const next = { ...prev, duration: dur, width: w, height: h, thumbnail: thumbPath, probing: false }
  ...
  function applyBulkProbe(results: Array<[string, number, number, number]>) {
    ...
    for (const [fp, dur, w, h] of results) {
      ...
      const prev = clips.value[i]
      const next = { ...prev, duration: dur, width: w, height: h, probing: false }
      next._search = buildSearch(next)
      clips.value[i] = markRaw(next)
      if (dur > 0) liveMeta.set(prev.id, { duration: dur, width: w, height: h })
    }
    scheduleClipsFlush()
  }

  function applyBulkProbeAndThumb(results: Array<[string, number, number, number, string, string]>) {
    if (!results.length) return
    const idxByFp = new Map<string, number>()
    for (let i = 0; i < clips.value.length; i++) idxByFp.set(clips.value[i].filepath, i)
    for (const [fp, dur, w, h, id, thumbPath] of results) {
      if (dur > 0) liveMeta.set(id, { duration: dur, width: w, height: h })
      if (thumbPath) liveThumbs.set(id, thumbPath)
      const i = idxByFp.get(fp)
      if (i === undefined) continue
      const prev = clips.value[i]
      const next = { ...prev, duration: dur, width: w, height: h, thumbnail: thumbPath || prev.thumbnail, probing: false }
      next._search = buildSearch(next)
      clips.value[i] = markRaw(next)
    }
    scheduleClipsFlush()
  ...
  async function scanFolderRecursive(folder: string) {
    importProgress.value = { current: 0, total: 0, active: true }
    try {
      await invoke('scan_folder_recursive', { folder })
    } catch (e) {
      console.error('scanFolderRecursive:', e)
    } finally {
  ...
  function injectSkeleton(tempId: string, filepath: string) {
    ...
    clips.value = [markRaw({
      ...
      created: '', createdTs: 0, duration: 0, width: 0, height: 0,
