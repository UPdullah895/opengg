...
watch(vu, (v) => { if (v > peakLevel.value) peakLevel.value = v; else peakLevel.value = Math.max(0, peakLevel.value - 1.2) })
const vuColor = computed(() => vu.value > 85 ? '#ef4444' : vu.value > 65 ? '#f59e0b' : props.color)

// Fader
const fillH = computed(() => Math.min(props.channel.volume, 150) / 150 * 100)
function faderInteract(e: MouseEvent, track: HTMLElement) {
  const rect = track.getBoundingClientRect()
  emit('update:volume', Math.round(Math.max(0, Math.min(150, (1 - (e.clientY - rect.top) / rect.height) * 150))))
}
function onFaderDown(e: MouseEvent) {
  e.preventDefault()
  const t = e.currentTarget as HTMLElement; faderInteract(e, t)
...
<template>
  <div class="strip" :class="[`strip--${type}`, { muted: channel.muted }]" :style="{ '--ch': color }">
    ...
    <!-- ★ FIX 3: fader-row takes up ALL available vertical space -->
    <div class="fader-row">
      <div class="vu">
        <div class="vu-track">
          <div class="vu-fill" :style="{ height:vu+'%', background:`linear-gradient(to top,${color}50,${vuColor})` }"></div>
      ...
      <div class="fader" @mousedown="onFaderDown">
        <div class="fader-track">
          ...
          <div class="fader-thumb" :style="{ bottom:`calc(${fillH}% - 7px)` }"><div class="grip"></div></div>
        </div>
      </div>
    </div>
...
.strip {
  ...
  /* ★ FIX 3: Let strip fill available height from parent */
  min-height: 0;
  height: 100%;
}
...
/* ★ FIX 3: fader-row grows to fill ALL available space — min 200px */
.fader-row {
  display: flex;
  gap: 5px;
  align-items: stretch;
  ...
  min-height: 200px;  /* ← guaranteed minimum for prominent sliders */
}
...
.vu-fill { position: absolute; bottom: 0; left: 0; right: 0; border-radius: 3px; transition: height 60ms linear; opacity: .85; }
.vu-peak { position: absolute; left: -1px; right: -1px; height: 2px; background: #fff; border-radius: 1px; transition: bottom 60ms linear; box-shadow: 0 0 4px rgba(255,255,255,.5); }
...
.fader-thumb { position: absolute; left: 50%; transform: translateX(-50%); width: 24px; height: 13px; background: linear-gradient(180deg, #e8e8e8, #c8c8c8); border-radius: 4px; box-shadow: 0 1px 3px rgba(0,0,0,.5), inset 0 1px 0 rgba(255,255,255,.3); cursor: grab; display: flex; align-items: center; justify-content: center; }
