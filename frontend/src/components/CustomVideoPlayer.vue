...
async function togglePlay() {
  ...
  playing.value ? videoRef.value.pause() : videoRef.value.play().catch(e => { if (import.meta.env.DEV) console.warn('play:', e) })
}
...
.cvp-prog-thumb {
  ...
  box-shadow: 0 0 6px rgba(0,0,0,.5); opacity: 1;
}

.cvp-row { display: flex; align-items: center; gap: 10px; }
.cvp-btn {
...
.cvp-time { font-size: 13px; color: rgba(255,255,255,.85); flex: 1; white-space: nowrap; }
.cvp-vol { display: flex; align-items: center; gap: 6px; }
.cvp-vol-ic { width: 18px; height: 18px; color: #fff; flex-shrink: 0; }
.cvp-vol-sl { width: 80px; height: 4px; accent-color: var(--accent); cursor: pointer; }
.cvp-fs svg { width: 18px; height: 18px; }
</style>
